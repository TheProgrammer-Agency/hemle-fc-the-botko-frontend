<template>
  <div class="main-wrapper">

    <HeaderElement @togglenav="navOpen = !navOpen" @toggleSearch="searchOpen = !searchOpen" />

    <OffCanvasMobileMenu :class="{'show-mobile-menu' : navOpen}" @togglenav="navOpen = !navOpen" />

    <SearchPopup :class="{'search-popup-open' : searchOpen}" @toggleSearch="searchOpen = !searchOpen" />

    <Breadcrumb :items="items" title="Direction" />


    <div class="bk-blog-grid-area pt--70 pb--100 pt_md--80 pb_md--80 pb_sm--80 pt_sm--60 bg_color--5">
      <div class="container" v-if="hasBlog">
        <div class="row">
          <div class="col-lg-8" >
            <div class="row">
              <div class="col-sm-6 move-up wow mt--30" v-for="blog in blogs" :key="blog.id">
                <div class="blog-grid">
                  <div class="post-thumb">
                    <n-link :to="localePath(`/blog/${blog.uuid}`)" >
                      <img :src="blog.image" :alt="blog.title[$i18n.locale]">
                    </n-link>
                  </div>
                  <div class="post-content">
                    <div class="post-inner">
                      <h5 class="heading heading-h5">
                        <n-link :to="localePath(`/blog/${blog.uuid}`)">{{ blog.title[[$i18n.locale]] }}</n-link>
                      </h5>

                      <p v-html="blog.description[[$i18n.locale]]">

                      </p>
                      <div class="post-meta">
                        <div class="post-date">{{ toDate(blog.created_at) }}</div>


                        <!--                       <div class="post-category">
                                                  <n-link :to="localePath(`/blog/category`)" >blog</n-link>
                                                </div>-->
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>



<!--            <div class="row">
              <div class="col-lg-12">
                <div class="brook-pagination-wrapper text-center pt&#45;&#45;80 pt_md&#45;&#45;40 pt_sm&#45;&#45;40">
                  <paginate
                      :page-count="4"
                      :page-range="3"
                      :margin-pages="2"
                      :prev-text="'Prev'"
                      :next-text="'Next'"
                      :container-class="'brook-pagination'"
                      :page-class="'page-item'">
                  </paginate>
                </div>
              </div>
            </div>-->
          </div>


<!--
          <div class="col-lg-4 mt_md&#45;&#45;40 mt_sm&#45;&#45;40 mt&#45;&#45;30">
            <BlogSidebar />
          </div>-->
        </div>
      </div>

      <div class="container" v-if="!hasBlog">

        <div class="col-lg-12">

          <div class="card-error"  >




            <div class="card-error-left">

              <h2>   {{
                  $t('blog.unavailable_title')
                }}
              </h2>

              <p>

                {{
                  $t('blog.unavailable_desc')
                }}


              </p>

              <n-link :to="localePath('index')" class="bk-btn theme-btn">{{$t('tools.btn.return_to_home')}} </n-link>

            </div>


            <div class="card-error-right">

              <img src="/img/home/notfound.svg" alt="">

            </div>
          </div>
        </div>

      </div>
    </div>

    <FooterTwo />

  </div>
</template>

<script>
import data from '~/data/blog.json';
import {slugify} from '~/mixins/slugify'
import {mapActions, mapGetters} from "vuex"
import moment from "moment";

export default {
  mixins: [slugify],
  auth:false,


  data () {
    return {
      data,
      navOpen: false,
      searchOpen: false,
      items: [
        {
          text: 'Home',
          to: "/"
        },
        {
          text: 'Blog',
          to: "/blog"
        },
        {
          text: 'Blog',
          active: true
        }
      ]
    }
  },

  components: {
    HeaderElement: () => import('@/components/HeaderElement'),
    OffCanvasMobileMenu: () => import('@/components/OffCanvasMobileMenu'),
    SearchPopup: () => import('@/components/SearchPopup'),
    Breadcrumb: () => import('@/components/Breadcrumb'),
    BlogSidebar: () => import('@/components/BlogSidebar'),
    FooterTwo: () => import('@/components/FooterTwo'),
  },
  computed:{
    ...mapGetters({
      blogs: 'blog/blogs'
    })
    ,
    hasBlog: function () {
      // `this` points to the vm instance
      return this.blogs.length > 0;
    }
  },
  async fetch() {

    try {

      await this.$store.dispatch('blog/allBlogs')

    } catch (errors) {
      // Set validation errors on a form

      console.log("une errror se produit", errors.data)
    }

  },

  mounted () {


    document.body.classList.add('template-color-1', 'template-font-1')
  },


  methods:{

    toDate($date){

      let d=new Date($date);

      return moment(d).format('MMMM Do YYYY')

    }
  },

  head() {
    return {
      title: 'Blog'
    }
  },
};
</script>